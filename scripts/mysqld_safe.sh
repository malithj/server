...
find_in_bin() {
  if test -x "$MY_BASEDIR_VERSION/bin/$2"
  then
    $1="$MY_BASEDIR_VERSION/bin/$2"
  elif test -x "@bindir@/$2"
  then
    $1="@bindir@/$2"
  else
    $1="$2"
...
set_mysqld_location_variable() {
  if test "$unsafe_my_cnf" = 0 -o \
          "$unrecognized_handling" = collect; then
    $1="$2"
  else
    log_error "$1=$2 was found in config file, when my.cnf is in the datadir"
    exit 1
  fi
}
...
parse_arguments() {
  for arg do
	...
	;;
      ...
      --ledir=*) set_mysqld_location_variable ledir "$val" ;;
      --malloc-lib=*) set_mysqld_location_variable malloc_lib "$val" ;;
      --crash-script=*) set_mysqld_location_variable crash_script "$val" ;;
      --mysqld=*)
        set_mysqld_location_variable mysqld "$val"
        MYSQLD="$val"
        ;;
      --mysqld-version=*)
        if test -n "$val"
        then
          set_mysqld_location_variable mysqld_version "$val"
          MYSQLD="mysqld-$val"
          PLUGIN_VARIANT="/$val"
        else
          MYSQLD="mysqld"
...
find_in_bin helper mysqld_safe_helper
find_in_bin print_defaults my_print_defaults
